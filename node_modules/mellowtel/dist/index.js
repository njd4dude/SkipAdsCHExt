...
// src/constants.ts
var MELLOWTEL_VERSION, MAX_PARALLEL_EXECUTIONS, MAX_PARALLEL_EXECUTIONS_BATCH, MAX_QUEUE_SIZE, LIFESPAN_IFRAME, DATA_ID_IFRAME, DATA_ID_IFRAME_BATCH, RULE_ID_XFRAME, RULE_ID_CONTENT_DISPOSITION, RULE_ID_CONTENT_TYPE, RULE_ID_VALUE_TO_MODIFY_CONTENT_TYPE_TO, RULE_ID_POST_REQUEST, MAX_DAILY_RATE, REFRESH_INTERVAL;
var init_constants = __esm({
  "src/constants.ts"() {
    "use strict";
    MELLOWTEL_VERSION = "1.3.1";
...
function insertInQueue(dataPacket, BATCH_execution) {
  return __async(this, null, function* () {
    return new Promise((resolve) => {
      let queueKey = BATCH_execution ? "queue_batch" : "queue";
      getLocalStorage(queueKey).then((result) => {
        if (result === void 0 || !result.hasOwnProperty(queueKey))
          result = { [queueKey]: [] };
...
function getLastFromQueue(BATCH_execution) {
  return __async(this, null, function* () {
    return new Promise((resolve) => {
      let queueKey = BATCH_execution ? "queue_batch" : "queue";
      getLocalStorage(queueKey).then((result) => {
...
function disableHeadersForPOST() {
  return new Promise(function(res) {
    chrome.declarativeNetRequest.updateSessionRules({
...
// src/utils/utils.ts
function getFrameCount(BATCH_execution) {
...
function fromDataPacketToNecessaryElements(dataPacket) {
  console.log(
  ...
  let saveHtml = dataPacket.hasOwnProperty("saveHtml") ? dataPacket.saveHtml.toString().toLowerCase() === "true" : false;
  let saveMarkdown = dataPacket.hasOwnProperty("saveMarkdown") ? dataPacket.saveMarkdown.toString().toLowerCase() === "true" : false;
  ...
  let waitForElementTime = dataPacket.hasOwnProperty("waitForElementTime") ? parseInt(dataPacket.waitForElementTime) : 0;
...
function preProcessCrawl(dataPacket, POST_request = false, BATCH_execution = false, batch_id = "") {
  return __async(this, null, function* () {
    ...
    if (POST_request) {
      ...
      for (let i = 0; i < index_to_arrive; i++) {
        let dataPacket2 = dataPacketArray[i];
        let {
          fastLane: fastLane2,
          orgId: orgId2,
          recordID: recordID2,
          ...
          triggersDownload,
          skipHeaders
        } = fromDataPacketToNecessaryElements(dataPacket2);
        promiseArray.push(
          crawlP2P(
            url,
            recordID2,
      ...
      if (BATCH_execution) {
        for (let i = index_to_arrive; i < dataPacketArray.length; i++) {
          ...
          yield insertInQueue(dataToBeQueued, BATCH_execution);
        }
      }
...
function crawlP2P(url, recordID, waitBeforeScraping, saveHtml, saveMarkdown, removeCSSselectors, classNamesToBeRemoved, fastLane, waitForElement, waitForElementTime, removeImages, htmlTransformer, isPDF, saveText, orgId, shouldSandbox, sandBoxAttributes, triggersDownload, skipHeaders, BATCH_execution, batch_id = "") {
  return new Promise((resolve) => {
    ...
    Promise.all([
      ...
      let max_parallel_executions = BATCH_execution ? MAX_PARALLEL_EXECUTIONS_BATCH : MAX_PARALLEL_EXECUTIONS;
...
function proceedWithActivation(url, recordID, eventData, waitForElement, shouldSandbox, sandBoxAttributes, BATCH_execution, triggerDownload = false, skipHeaders = false, hostname = "") {
  return __async(this, null, function* () {
    if (triggerDownload) {
      yield sendToBackgroundToSeeIfTriggersDownload(url, triggerDownload);
    }
    if (skipHeaders) {
      yield disableXFrameHeaders(hostname, skipHeaders);
    }
...
function resetAfterCrawl(recordID, BATCH_execution) {
  return __async(this, null, function* () {
    let dataPacket = yield getLastFromQueue(BATCH_execution);
    if (dataPacket && dataPacket.url !== "") {
      ...
      setTimeout(() => {
        ...
        Logger.log(
          "[\u{1F310}] : frameCountTotal in cleanUpAfterCrawl (before resetting headers)  => " + frameCountTotal
        );
        if (frameCountTotal === 0 && !BATCH_execution) {
          Logger.log("[\u{1F310}] : Resetting headers!");
          enableXFrameHeaders("");
          resetTriggersDownload();
        } else if (frameCountTotal === 0 && BATCH_execution) {
          setTimeout(() => {
            Logger.log("[\u{1F310}] : Resetting headers (BATCH_execution)!");
            enableXFrameHeaders("");
            resetTriggersDownload();
          }, 6e4);
        }
...
function setLifespanForIframe(recordID, waitBeforeScraping, BATCH_execution) {
  Logger.log(
    "Setting lifespan for iframe => " + (LIFESPAN_IFRAME + waitBeforeScraping) + " ms. RecordID => " + recordID
  );
  ...
  "src/local-rate-limiting/rate-limiter.ts"() {
    ...
    RateLimiter = class {
      static getRateLimitData() {
        return __async(this, null, function* () {
          let timestamp = yield getLocalStorage("timestamp_mellowtel");
          if (timestamp === void 0 || !timestamp.hasOwnProperty("timestamp_mellowtel")) {
            timestamp = void 0;
          } else {
            timestamp = parseInt(timestamp.timestamp_mellowtel);
          }
          let count = yield getLocalStorage("count_mellowtel");
          if (count === void 0 || !count.hasOwnProperty("count_mellowtel")) {
            count = void 0;
          } else {
      ...
      static setRateLimitData(timestamp, count) {
        return __async(this, null, function* () {
          yield setLocalStorage("timestamp_mellowtel", timestamp);
          yield setLocalStorage("count_mellowtel", count);
        });
      }
      static getLifetimeTotalCount() {
        return __async(this, null, function* () {
          let lifetime_total_count = yield getLocalStorage(
            "lifetime_total_count_mellowtel"
          );
          if (lifetime_total_count === void 0 || !lifetime_total_count.hasOwnProperty("lifetime_total_count_mellowtel")) {
      ...
      static setHistoricData(initial_timestamp, lifetime_total_count) {
        return __async(this, null, function* () {
          yield setLocalStorage("initial_timestamp_mellowtel", initial_timestamp);
      ...
      static resetRateLimitData(now, add_to_count = false) {
        return __async(this, null, function* () {
          yield this.setRateLimitData(now, add_to_count ? 1 : 0);
        });
      }
      static getIfRateLimitReached() {
        return __async(this, null, function* () {
          let mllwtl_rate_limit_object = yield getLocalStorage(
            "mllwtl_rate_limit_reached"
          );
          if (mllwtl_rate_limit_object === void 0 || !mllwtl_rate_limit_object.hasOwnProperty("mllwtl_rate_limit_reached")) {
            return false;
          } else {
      ...
      static checkRateLimit() {
        return __async(this, null, function* () {
          ...
          let { lifetime_total_count } = yield this.getLifetimeTotalCount();
          if (!timestamp) {
            Logger.log(
              `[\u{1F552}]: NO_TIMESTAMP, setting timestamp, count, and historic data`
            );
            yield this.setRateLimitData(now, 1);
          ...
          if (elapsedTime > REFRESH_INTERVAL) {
            Logger.log(`[\u{1F552}]: REFRESH_INTERVAL elapsed, resetting count`);
            yield this.resetRateLimitData(now, true);
    ...
    RateLimiter.MAX_DAILY_RATE = MAX_DAILY_RATE;
  }
});
...
var init_measure_connection_speed = __esm({
  "src/utils/measure-connection-speed.ts"() {
    "use strict";
    init_logger();
...
function startConnectionWs(identifier) {
  return __async(this, null, function* () {
    yield getSharedMemory("webSocketConnectedMellowtel").then(
      (response) => __async(this, null, function* () {
        if (!response) {
          let LIMIT_REACHED = yield RateLimiter.getIfRateLimitReached();
          if (LIMIT_REACHED) {
            Logger.log(`[\u{1F310}]: Rate limit, not connecting to websocket`);
            let { timestamp, count } = yield RateLimiter.getRateLimitData();
            let now = Date.now();
            let timeElapsed = RateLimiter.calculateElapsedTime(now, timestamp);
            Logger.log(`[\u{1F310}]: Time elapsed since last request: ${timeElapsed}`);
            if (timeElapsed > REFRESH_INTERVAL) {
              Logger.log(
                `[\u{1F310}]: Time elapsed is greater than REFRESH_INTERVAL, resetting rate limit data`
              );
              yield setLocalStorage("mllwtl_rate_limit_reached", false);
              yield RateLimiter.resetRateLimitData(now, false);
            ...
            ws.onmessage = function incoming(data) {
              return __async(this, null, function* () {
                if (yield isMellowtelStarted()) {
                  ...
                  if (data.hasOwnProperty("type_event") && data.type_event === "heartbeat") {
                    return;
                  }
                  ...
                  let { shouldContinue, isLastCount } = yield RateLimiter.checkRateLimit();
                  if (shouldContinue || POST_request || BATCH_execution) {
                    if (isLastCount && !POST_request && !BATCH_execution) {
                      Logger.log(`[\u{1F310}]: Last count reached, closing connection...`);
                      yield setLocalStorage("mllwtl_rate_limit_reached", true);
                      ws.close();
                    }
                    yield preProcessCrawl(
                      data,
                      POST_request,
                      BATCH_execution,
                      batch_id
                    ...
                    Logger.log("[\u{1F310}]: Rate limit reached, closing connection...");
                    yield setLocalStorage("mllwtl_rate_limit_reached", true);
                    ws.close();
                  }
                }
...
function handlePostRequest(method_endpoint, method_payload, method_headers, fastLane, orgId, recordID) {
  return new Promise(function(res) {
    return __async(this, null, function* () {
      yield disableHeadersForPOST();
      ...
      fetch(method_endpoint, requestOptions).then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
        return response.json();
      }).then((data) => __async(this, null, function* () {
        Logger.log("Response from server:", data);
        yield saveJSON(recordID, data, orgId, fastLane, method_endpoint);
        res(data);
      })).catch((error) => {
...
function saveJSON(recordID, json, orgId, fastLane, endpoint) {
  return __async(this, null, function* () {
    return new Promise(function(res) {
      const targetUrl = "https://afcha2nmzsir4rr4zbta4tyy6e0fxjix.lambda-url.us-east-1.on.aws/";
      const requestOptions = {
        method: "POST",
        headers: {
      ...
      fetch(targetUrl, requestOptions).then((response) => __async(this, null, function* () {
        if (!response.ok) {
          yield enableHeadersForPOST();
          throw new Error("Network response was not ok");
        }
        return response.json();
      })).then((data) => __async(this, null, function* () {
...
function removeSelectorsFromDocument(document2, selectorsToRemove) {
...
function tellToDeleteIframe(recordID, BATCH_execution) {
  return new Promise((resolve) => {
    sendMessageToBackground({
      intent: "deleteIframeMellowtel",
      recordID,
...
function saveCrawl(recordID, content, markDown, fastLane, url, htmlTransformer, orgId, saveText, BATCH_execution, batch_id) {
...
var init_pdf_getter = __esm({
  "src/pdf/pdf-getter.ts"() {
    "use strict";
    PDFJS = __toESM(require("pdfjs-dist/build/pdf.js"));
    PDFJSWorker = __toESM(require("pdfjs-dist/build/pdf.worker.js"));
    init_logger();
  }
});
...
function initCrawlHelper(event, numTries) {
  ...
  if (isMellowtelCrawl) {
    ...
    let fastLane = event.data.hasOwnProperty("fastLane") ? event.data.fastLane : false;
    let url_to_crawl = event.data.hasOwnProperty("url_to_crawl") ? event.data.url_to_crawl : "";
    let htmlTransformer = event.data.hasOwnProperty("htmlTransformer") ? event.data.htmlTransformer : "none";
    let removeImages = event.data.hasOwnProperty("removeImages") ? event.data.removeImages.toString().toLowerCase() === "true" : false;
    ...
    setTimeout(() => __async(this, null, function* () {
      ...
      let saveText = event.data.hasOwnProperty("saveText") ? event.data.saveText : false;
      ...
      if (removeCSSselectors === "default") {
        ...
        try {
          ...
          Logger.error(
            "[initCrawl \u{1F310}] : Error parsing removeCSSselectors =>",
            e
          );
        }
      }
      ...
      if (!isPDF) {
        ...
        if ((markDown.trim() === "" || markDown === "null") && numTries < 4) {
          Logger.log("[initCrawl \u{1F310}] : markDown is empty. RESETTING");
          setTimeout(() => {
          ...
          saveCrawl(
            ...
            orgId,
            saveText,
            event.data.hasOwnProperty("BATCH_execution") ? event.data.BATCH_execution : false,
...
init_logger();
var mellowtelRequiredPermissions = [
  "storage",
  "tabs",
  "declarativeNetRequest"
];
function checkIfInPermissions(permission, callback) {
  const manifest = chrome.runtime.getManifest();
  const permissions = manifest.permissions || [];
  callback(permissions.includes(permission));
}
function checkIfInOptionalPermissions(permission, callback) {
  const manifest = chrome.runtime.getManifest();
  const optionalPermissions = manifest.optional_permissions || [];
  callback(optionalPermissions.includes(permission));
}
function checkRequiredPermissions(requestAfterChecking = false) {
  return __async(this, null, function* () {
    ...
    for (let permission of mellowtelRequiredPermissions) {
      ...
      checkIfInPermissions(permission, (isPresent) => {
        if (isPresent) {
          ...
          checkIfInOptionalPermissions(permission, (isPresent2) => {
            if (isPresent2) {
              permissionsToRequest.push(permission);
              if (permission === "declarativeNetRequestWithHostAccess") {
                hostPermissions = chrome.runtime.getManifest().optional_host_permissions || ["https://*/*"];
              }
            }
          });
        }
    ...
    if (requestAfterChecking && permissionsToRequest.length > 0) {
      let alreadyGranted = yield checkIfPermissionsGranted(permissionsToRequest);
      if (!alreadyGranted) {
        let granted = yield requirePermissions(
...
function requirePermissions(_0) {
  return __async(this, arguments, function* (permissions, hostPermissions = []) {
    return new Promise((resolve) => {
...
function checkIfPermissionsGranted(permissions) {
  return __async(this, null, function* () {
    return new Promise((resolve) => {
...
var Mellowtel = class {
  constructor(publishableKey, options) {
    ...
    this.MAX_DAILY_RATE = MAX_DAILY_RATE;
    this.publishableKey = publishableKey;
    this.options = options;
    this.disableLogs = (options == null ? void 0 : options.disableLogs) !== void 0 ? options.disableLogs : true;
    this.MAX_DAILY_RATE = (options == null ? void 0 : options.MAX_DAILY_RATE) || MAX_DAILY_RATE;
  ...
  initBackground(auto_start_if_opted_in, metadata_id) {
    return __async(this, null, function* () {
      ...
      yield checkRequiredPermissions(false);
  ...
  start(metadata_id) {
    return __async(this, null, function* () {
      return new Promise((resolve) => __async(this, null, function* () {
        ...
        try {
          yield checkRequiredPermissions(true);
